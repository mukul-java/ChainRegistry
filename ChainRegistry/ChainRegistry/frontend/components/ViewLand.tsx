...
export default function ViewDetail({
	...
	const handleDownloadPDF = () => {
		...
		doc.setFontSize(18);
		doc.text("Land Paper", 14, 20);
	
		doc.setFontSize(12);
		doc.text(`Land ID: ${data.id}`, 14, 35);
		doc.text(`Area: ${data.area} Sq.Ft`, 14, 45);
		doc.text(`Address: ${data.address}`, 14, 55);
		doc.text(`Verified: ${data.isLandVerified ? "Yes" : "No"}`, 14, 65);
	
		doc.text("User Info:", 14, 80);
		doc.text(`Name: ${userInfo.name}`, 14, 90);
		doc.text(`Age: ${userInfo.age}`, 14, 100);
		doc.text(`Email: ${userInfo.email}`, 14, 110);
		doc.text(`Phone: ${userInfo.phone}`, 14, 120);
		if (userInfo.aadharNumber)
			doc.text(`Aadhar: ${userInfo.aadharNumber}`, 14, 130);
		if (userInfo.panNumber)
			doc.text(`PAN: ${userInfo.panNumber}`, 14, 140);
	
		doc.save(`Land-${data.id}-Details.pdf`);
	};
